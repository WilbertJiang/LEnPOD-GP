...
int main(int argc, char** argv) {
    ...
    double l = 0.0088;               // Length         
    double w = 0.01179375;               // Width
    double h = 7.2e-4;             // Height
    unsigned ls = 207;              // Meshes in the length axis]
    unsigned ws = 278;              // Meshes in the width axis
    unsigned hs = 17;               // Meshes in the height axis
    double T = 3*2000*5.33e-6;         // Final Time or Total Time (S)
    unsigned num_steps = 2000;      // Total number of time steps
    double t = 0;                 
    double delt = T/num_steps;      // Time step size

    double h_c = 2.40598e4;     
    double Ta = 0.0;                // Initial/Ambient Temperature
    unsigned Nu = 1;               // Number of functional units
    ...
    std::string ptrace_file = "../../power_SM_training.txt";
    std::string floorplan_file = "../../Core_flp.txt";
    ...
    for( unsigned i = 0; i < num_steps; i++ ) {
        for( unsigned j = 0; j < Nu; j++ ) {
            pd[i][j] = pd[i][2]/(flp[j][0]*flp[j][1]*thick_actl);
        }
    }
   ...
   //std::cout << "here is okay"<<std::endl;
    ...
    for(unsigned i = 0; i < num_steps; i++ ) {
        ...
        // save solution file 
        ss << i;
        solution_file_name = "../../solution_Core_train/file_" + ss.str() + "h5";
