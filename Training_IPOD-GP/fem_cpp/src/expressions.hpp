...
class KappaExpression : public Expression {
    public:
        ...
        double k_0=0.0, k_1=0.0, tol=0.0;

        void setParams(double tolin, double k_0in, double k_1in) {
            tol = tolin;
            k_0 = k_0in;
            k_1 = k_1in;
        }
        ...
        void eval(Array<double>& values, const Array<double>& x) const {
            values[0] = ( (x[2] <= 5.85e-4 + tol) ? k_0 : k_1);
        }    
};
...
class DS1Expression : public Expression {
    public:
        // make sure to set these, or there will be issues
        double D0=0.0, D1=0.0, tol=0.0;
        
        void setParams(double tolin, double D0in, double D1in) {
            tol = tolin;
            D0 = D0in;
            D1 = D1in;
        }
        ...
        void eval(Array<double>& values, const Array<double>& x) const {
            values[0] = ( (x[2] <= 5.85e-4 + tol) ? D0 : D1);
        }    
};
...
class SCExpression : public Expression {
    public:
        // make sure to set these, or there will be issues
        double c0=0.0, c1=0.0, tol=0.0;
        
        void setParams(double tolin, double c0in, double c1in) {
            tol = tolin;
            c0 = c0in;
            c1 = c1in;
        }
        ...
        void eval(Array<double>& values, const Array<double>& x) const {
            values[0] = ( (x[2] <= 5.85e-4 + tol) ? c0 : c1);
        }    
};
...
class SourceTerm : public Expression {
    public:
        ...
        void eval(Array<double>& values, const Array<double>& x) const {
            if( (x[2] > (5.85e-4-thick_actl)) && (x[2] < 5.85e-4)) {
		    values[0] = 0.0;
